...
char commands[command_len][20] = {
	...
	"read",		//11	//read_id
	"unread",	//12	//unread_id
	"emoji",	//13	//emoji_Emoji-1,Emoji-2,Emoji-3_id
	"rememoji", //14    //rememoji_Emoji-1,Emoji-2,Emoji-3_id
	"notemoji", //15	//notemoji_Emoji-1,Emoji-2,Emoji-3_id
};
...
int process_output(SOCKET localClientSocket, char name[15], int clientnumber, int num, char word1[DEFAULT_BUFLEN], char word2[DEFAULT_BUFLEN]) {
	...
	case 1://send many person
		...
		for (int i = 0; i < number; i++) {
			...
			if (strcmp(name, names[i]) == 0) {
				...
				sendd(localClientSocket, alertmessage, name);
				continue;
			}
			gettime(sent);
		label:
			...
			if (value) {
				...
				if (valueClientSocket != INVALID_SOCKET) {
					...
					if (!sendd(valueClientSocket, tonewmessage, names[i])) goto label;//Incase sent failure repeat from label
	...
	case 3: //login
		...
		//word2 -> password
		if (num == 2) {
			boolean = myfiles->messagefile.loginid.Find(word1);
	...
	case 15: // not emoji
		...
		alertboolean = myfiles->messagefile.Edit(word2, name, number, letter);
		break;
	}
